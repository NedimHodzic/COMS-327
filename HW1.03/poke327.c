...
typedef struct map {
  ...
  int8_t n, s, e, w;
  //Added in a new weight array for the map.
...
//Global variables for the player characters position.
...
//Algorith used to print out the distance map, very similar to what Shaefer used for the paths but used weights of each tile and added more if statements for the diagonal directions.
...
//This places the character using 2 global variables.
static int place_character(map_t *m) {
    while(1) {
        playerX = (rand() % ((MAP_X - 1))) + 1;
        playerY = (rand() % ((MAP_Y - 1))) + 1;
        if(m->map[playerY][playerX] == ter_path) {
            m->map[playerY][playerX] = person_pc;
            break;
        }
    }
...
//This sets the weights for the hiker.
static int setWeightHiker(map_t *m) {
    int x,y;
...
//This sets the weights for the rival.
static int setWeightRival(map_t *m) {
    int x,y;
...
int main(int argc, char *argv[])
{
  ...
  //Prints the distance maps
  setWeightHiker(world.cur_map);
  dijkstraDistanceMap(world.cur_map);
  setWeightRival(world.cur_map);
  dijkstraDistanceMap(world.cur_map);
